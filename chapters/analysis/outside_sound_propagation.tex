...
In equation \autoref{eq:ana:near_field} it can be calculated that less than \Hz{80} radiate directly intro spherical wave on the exit of the line source array no matter the hight. The following \autoref{fig:ana:near_far_field} shows a vertical cut of the near-field and far-field from a line source array. 

\fig{near_far_field}{The figure shows a vertical cut of a sound wave radiation pattern of a line source array \citep{bauman2001wavefront}.}{fig:ana:near_far_field}{0.80}

As seen in \autoref{fig:ana:near_far_field}, the wave propagate as planar wave in the near-field. In the vertical domain, the plane wave propagates as a cylindrical wave in the near-field, where the coverage area for every double of distance is twice as big. Since the coverage area is twice as big, the \gls{spl} is \dBr{-3} for the doubled distance. When the wave excites distance $d_B$, the wave propagates intro far-field where the coverage area is four times higher while travelling the double of distance and therefore the \gls{spl} is \dBr{-6}. In far-field, the wave propagates as a point source. The following \autoref{fig:ana:KUDO_nearfield_limit} gives two examples of the $d_B$ distance with different line source array hight.

\plot{plot/KUDO_nearfield_limit}{The figure shows two hight example calculated from \autoref{eq:ana:near_field}. The line indicate the distance where the sound wave goes from near-field to far-field.}{fig:ana:KUDO_nearfield_limit}
...
\fig{temp_worst_case}{The graph shows the attenuation in \si{\decibel} \gls{spl} with respect to frequency, humidity and temperature \citep{corteel2017large}.}{fig:ana:temp_eg}{0.9}
...
% = ||c \cdot n + v||_2 = ||v_{ray}||_2

\startexplain
\explain{\theta}{is the angle between the sound direction and the wind direction.}{\degree}
...
As shown in shell's law \autoref{eq:ana:shell_law}, the frequency dependency is not a factor. The article \citep{review_of_sound} only explorer frequency up to \Hz{3200}, but since the refraction depends on the wavelength, the distance of refraction wave might be smaller for higher frequency. The attenuation with respect to refraction seems to have a saddle attenuation at \dBr{20}. A measurement in \citep{review_of_sound} shows the attenuation for the centre frequency of \Hz{1200} with \octave{3} band filtered aircraft noise over mown grass. The measurement is interesting with respect to a concert area and is therefore shown in \autoref{fig:ana:excess} 
...
\xfig{analysis/refraction_temp.pdf_t}{Wave refraction of an omnidirectional line source array in inhomogeneous temperature with lapse profile.}{fig:ana:temp_ref}{0.9}
...
\xfig{analysis/refraction.pdf_t}{Wave refraction of a omnidirectional line source array in inhomogeneous logarithmically increasing wind profile where the wind gradient points towards left.}{fig:ana:wind_ref}{0.9}
...
\xfig{analysis/measurement_one_sec.pdf_t}{The figure shows the microphone position versus the position of the line source array and the wind direction.}{fig:ana:position}{0.8}   
...
\plot{plot/measurement_two_one_ana}{The graph shows the first transfer function measurement within the high frequency directional angle of the line source array. The $L_{eq}$ \gls{spl} different between the microphones is \dBr{4.41}.}{fig:ana:mea_two_one}
...
As it is shown in \autoref{ta:ana:spl_dif}, the $L_{eq}$ \gls{spl} is higher for microphone 1 in all measurement. Moreover the average $L_{eq}$ \gls{spl} difference is \dBr{4.67}, where for A-weighted $L_{Aeq}$ \gls{spl} the average difference is \dBr{6.17}. With respect to the intelligibility frequency range, a weighting filter is designed to observe the \gls{spl} differences in the critical intelligibility frequency range. The filter is based on the founded intelligibility frequency range in \citep{arl_us_army}. It is shown in \citep{arl_us_army} that the critical intelligibility frequency range lays between \Hz{1000} and \Hz{4000}. The designed intelligibility weighting filter is an 8\textsuperscript{th} order bandpass filter with lower crossover frequency at \Hz{1000} and higher crossover frequency at \Hz{4000}. The resulting average difference is \dBr{7.88} and the maximum difference is \dBr{9.95}.
...
% The fluctuation increases with distance until the standard divination of the phase fluctuation is comparable to \SI{90}{\degree} \citep{review_of_sound}. At this point, the phase correlation for each sound path is uncorrelated.
