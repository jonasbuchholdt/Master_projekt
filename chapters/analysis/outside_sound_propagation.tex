...
Since the study of crosswind effects is rare, a geometric ray trace calculation of the effect is done. The geometric calculation will only be done for crosswind which mean that the wind is orthogonal the the frontal direction of the speaker. It will be based on the directional speaker with \SI{90}{\degree} angle, so \SI{45}{\degree} angle from frontal direction as the \autoref{fig:ana:spe_ang} shows.



The study of crosswind effect on directional speakers is rare, but a cuple of author have has studied the effect on a omnidirectional source. The author of \citep{ray_tracing} implemented a ray tracing method with a vector based interpolation as shown in \autoref{fig:ana:cal_wav_dir}.
...
As the wave propagating, the resulting $v_{ray}$ increases in the direction of the wind. The article \citep{crosswind_simulation} simulates the effect of crosswind in a \gls{fdtd} simulation. The used wind speed is \SI{102.9}{\meter\per\second} so for normal condition at a concert the effect will be much less. The following simulation \autoref{fig:ana:cro_win_sim} shows the simulation result from \citep{crosswind_simulation}. The source is a omnidirectional \Hz{100} espherical source while the wind have a constant uniform wind speed from left. The simulation is done is two dimentions. 
...
It can be seen in \autoref{fig:ana:cro_win_sim} that the cross wind do not effect the direction of the wave, it only effect the time of arrival to the resiever.
