...
\fig{IMG_0107}{The picture shows the measuring setup while the parallel wind measuring is under preperation.}{fig:meas:setup_shown}{1}
...
\plot{plot/measuring_1_0_deg}{The graph shows the average frequency response of 10 measurements for all three microphone while the speaker is not rotated. The average is calculated in the time domain by aligning the impulse response with the help of cross-correlation. The wind speed and wind direction is also the average of the 10 measurements.}{fig:td:mea_1_0_deg}
...
While using the \SI{5}{\degree} forward tilt angle along with the measurement, the microphone exit above the near-field of the line source array coverage main lobe. The near-field upper limit is calculated to be \SI{63}{\centi\meter} above the ground in the distance of the microphone. Therefore, since the microphone is placed \SI{1.68}{\meter} above the ground, the microphone is fare above the near-field. Comparing the \autoref{fig:td:mea_1_0_deg} and \autoref{fig:td:mea_1_0_deg_down} it is clearly seen that the microphone is outside the near-field of the high frequency. Above \Hz{2000}, the \gls{spl} is more than \dB{10} lower in the ear measuring hight compare to the ground position with the same distances to the speaker. 
...
\begin{requirement}\label{req:speaker_directionality}
    ...
    \argument{This requirement is made according to the frequency response in the measuring result \autoref{sec:des:measuring_result}, where the differences in directionality characteristics make it difficult to compare the upwards microphone and downwards microphone since the line source array has a \gls{spl} boost in the \SI{45}{\degree}. }
\end{requirement}
